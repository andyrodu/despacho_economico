...
if st.button("Descargar demanda"):
    with st.spinner("Descargando (con batching + caché)..."):
        try:
            ...
            st.error("Falló la descarga. Abajo va el error REAL (sin redacción):")
            st.exception(e)  # <- esto es clave
            st.info("Tip: si el error muestra HTML/Forbidden/Cloudflare, es bloqueo del sitio a Streamlit Cloud.")
            st.stop()

    st.success(
        f"Listo. Filas: {q['rows']} | NaNs: {q['nans']} | Neg: {q['negatives']} "
        f"| Duplicados: {q['duplicates']} | Missing hours: {q['missing_hours']}"
    )
